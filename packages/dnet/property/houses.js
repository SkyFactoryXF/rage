...
houses.loadAll = function() {
    ...
    mysql.executeQuery(`SELECT * FROM houses`, function (err, rows, fields) {
        rows.forEach(function(item) {
            ...
            mp.labels.new(`${item['id']}`, new mp.Vector3(pos.x, pos.y, pos.z + 1.0), {
                dimension: 0,
                font: 4,
                los: true,
                drawDistance: 20
...
houses.enterv = function (player, id) {
    ...
    if (vehicles.exists(v) && player.vehicle) {
        ...
        if (vehicles.exists(player.vehicle)) {
            player.vehicle.position = new mp.Vector3(houses.garageList[garageId][4], houses.garageList[garageId][5], houses.garageList[garageId][6]);
