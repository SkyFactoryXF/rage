...
pickups.BotLspd1 = new mp.Vector3(441.10565185546875, -981.8861694335938, 29.689579010009766);
...
pickups.DispatcherPos1 = new mp.Vector3(447.6112365722656, -998.8116455078125, 34.97017288208008 - 1.0);
pickups.DispatcherPos2 = new mp.Vector3(445.4342956542969, -998.9093627929688, 34.97017288208008 - 1.0);
pickups.DispatcherPos3 = new mp.Vector3(443.2780456542969, -998.9052734375, 34.970130920410156 - 1.0);
...
pickups.SapdGarderobPos = new mp.Vector3(461.8287353515625, -996.461181640625, 30.689584732055664 - 1.0);
pickups.SapdArsenalPos = new mp.Vector3(482.52978515625, -995.8043823242188, 30.689804077148438 - 1.0);
pickups.SapdClearPos = new mp.Vector3(453.9168701171875, -988.1390380859375, 30.68958282470703 - 1.0);
pickups.SapdArrestPos = new mp.Vector3(475.06805419921875, -1015.2300415039062, 26.273454666137695 - 1.0);
pickups.SapdPCnew = new mp.Vector3(473.7437438964844, -1005.874755859375, 26.273462295532227 - 1.0);
...
//elevator
pickups.UsmcPickupPos1 = new mp.Vector3(-2360.705322265625, 3249.455322265625, 32.810707092285156 - 1.0);
pickups.UsmcPickupPos2 = new mp.Vector3(-2360.73779296875, 3249.52294921875, 92.90374755859375 - 1.0);
...
pickups.UsmcArsenalPos1 = new mp.Vector3(-2357.2685546875, 3255.92578125, 32.810707092285156 - 1.0);
pickups.UsmcArsenalPos2 = new mp.Vector3(-2358.54541015625, 3254.180419921875, 32.810707092285156 - 1.0);
pickups.UsmcStockPos = new mp.Vector3(-2345.516357421875, 3232.524169921875, 34.752037048339844 - 1.0);

pickups.UsmcKeyPos = new mp.Vector3(-2348.9111328125, 3245.7138671875, 32.95459747314453 - 1.0);
pickups.UsmcInfoPos = new mp.Vector3(-2347.100830078125, 3269.737060546875, 32.81076431274414 - 1.0);


pickups.SapdStockPos = new mp.Vector3(474.90301513671875, -995.3037719726562, 26.2734375 - 1.0);
...
pickups.SapdInfoPos = new mp.Vector3(462.4835510253906, -985.3841552734375, 30.728052139282227 - 1.0);
...
/*Gang*/
pickups.GangUserPos1 = new mp.Vector3(-185.1627, -1702.005, 31.76884); //grove
pickups.GangUserPos2 = new mp.Vector3(85.6908, -1959.55, 20.1217); //ballas
pickups.GangUserPos3 = new mp.Vector3(1297.2596435546875, -1618.038330078125, 53.53297805786133); //marabunda

//253.5638885498047, -1808.5946044921875, 26.113126754760742,
pickups.GangUserPos4 = new mp.Vector3(253.5638885498047, -1808.5946044921875, 26.113126754760742); //vagos

pickups.GangUserPos5 = new mp.Vector3(1092.794677734375, -2251.919921875, 30.233299255371094); //bloods

pickups.GangUserPosInt = new mp.Vector3(2737.886, -374.227, -48.98799); //idk
...
/*Mafia*/
pickups.laCosaNostra = new mp.Vector3(-1886.5675048828125, 2050.37890625, 140.9825897216797 - 1.0);
pickups.yakuza = new mp.Vector3(-1516.5753173828125, 851.6640014648438, 181.5946502685547 - 1.0);
pickups.russianMafia = new mp.Vector3(-60.54205322265625, 360.08062744140625, 113.05644989013672 - 1.0);
...
pickups.checkPressLAlt = function (player) {
    ...
    if (methods.distanceToPos(pickups.GangUserPosInt, playerPos) < distanceCheck && player.dimension === 6) {
        player.dimension = 0;
        user.teleport(player, pickups.russianMafia.x, pickups.russianMafia.y, pickups.russianMafia.z + 1);
    }
    if (methods.distanceToPos(pickups.GangUserPosInt, playerPos) < distanceCheck && player.dimension === 7) {
        player.dimension = 0;
        user.teleport(player, pickups.yakuza.x, pickups.yakuza.y, pickups.yakuza.z + 1);
    }
    if (methods.distanceToPos(pickups.GangUserPosInt, playerPos) < distanceCheck && player.dimension === 8) {
        player.dimension = 0;
        user.teleport(player, pickups.laCosaNostra.x, pickups.laCosaNostra.y, pickups.laCosaNostra.z + 1);
    }
    ...
    if (methods.distanceToPos(pickups.russianMafia, playerPos) < distanceCheck) {
        player.dimension = 6;
        user.teleport(player, pickups.GangUserPosInt.x, pickups.GangUserPosInt.y, pickups.GangUserPosInt.z + 1);
    }
    if (methods.distanceToPos(pickups.laCosaNostra, playerPos) < distanceCheck) {
        player.dimension = 7;
        user.teleport(player, pickups.GangUserPosInt.x, pickups.GangUserPosInt.y, pickups.GangUserPosInt.z + 1);
    }
    if (methods.distanceToPos(pickups.yakuza, playerPos) < distanceCheck) {
        player.dimension = 8;
        user.teleport(player, pickups.GangUserPosInt.x, pickups.GangUserPosInt.y, pickups.GangUserPosInt.z + 1);
    }
...
pickups.checkPressE = async function (player) {
    ...
    if (user.isSheriff(player) || user.isFib(player) || user.isSapd(player)) {
        ...
        if (methods.distanceToPos(pickups.SapdClearPos, playerPos) < distanceCheck)
            player.call('client:menuList:showSapdClearMenu');
        if (methods.distanceToPos(pickups.SapdPCnew, playerPos) < distanceCheck)
            player.call('client:menuList:showSapdClearMenu');
...
pickups.createAll = function () {
    ...
    methods.createCpVector(pickups.SapdPCnew, 'Нажмите ~g~E~s~ чтобы открыть меню', 1, -1, pickups.Blue);
    ...
    methods.createCpVector(pickups.laCosaNostra, 'Нажмите ~g~E~s~ чтобы войти', 1, -1, pickups.White);
    methods.createCpVector(pickups.yakuza, 'Нажмите ~g~E~s~ чтобы войти', 1, -1, pickups.White);
    methods.createCpVector(pickups.russianMafia, 'Нажмите ~g~E~s~ чтобы войти', 1, -1, pickups.White);

    for ( let i = 0; i < enums.WeedZones.length; i++) {
        methods.createCpVector(enums.WeedZones[i].position, 'Вы можете посадить сорняк здесь!', 15, -1, [255, 235, 59, 0]);
    }
};