...
timer.min10Timer = function() {
    ...
    if (weedplants.getAll().length !== 0) {
        weedplants.getAll().forEach((plant) => {
            if (plant.id !== -1 && plant.growngrams <= 49) {
                plant.growngrams++;
                weedplants.changeStage(plant.id);
                if (plant.id !== -1 && plant.growngrams === 50) {
                    weedplants.updateLabel(plant.id, `~g~Сорняк~w~~n~~n~Выросшие граммы: ${plant.growngrams}~n~Ready for pickup`);
                } else {
                    weedplants.updateLabel(plant.id, `~g~Сорняк~w~~n~~n~Выросшие граммы: ${plant.growngrams}`);
                }
            }
        });
    }
