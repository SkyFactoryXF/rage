...
user.startFight = (player, target, money) => {
    ...
    if (!fighting) return;
    if (user.get(fighting, 'money') < money) return fighting.notify('У тебя недостаточно денег, чтобы начать бой.');
    if (user.get(player, 'money') < money) return fighting.notify('У тебя недостаточно денег, чтобы начать бой.');

    player.call('client:user:freeze', [true]);
    fighting.call('client:user:freeze', [true]);

    let fightInterval = setInterval(() => {
        count--;
        player.notify(`${count}`);
        fighting.notify(`${count}`);

        if (count === 1) {
            player.notify(`GO GO GO!`);
            fighting.notify(`GO GO GO!`);

            player.setVariable("FIGHTING", true);
            player.setVariable("FIGHTING_PLAYER", target);
            player.setVariable("ROUNDS_WON", 0);
            player.setVariable("FIGHTING_MONEY", money);

            fighting.setVariable("FIGHTING", true);
            fighting.setVariable("FIGHTING_PLAYER", player.id);
            fighting.setVariable("ROUNDS_WON", 0);
            fighting.setVariable("FIGHTING_MONEY", money);

            user.showLoadDisplay(player);
            user.showLoadDisplay(fighting);

            setTimeout(() => {
                user.hideLoadDisplay(player);
                user.hideLoadDisplay(fighting);
            }, 2500);

            player.call('client:user:freeze', [false]);
            fighting.call('client:user:freeze', [false]);



            player.notify("У вас есть две минуты на бой, если вы не будете драться в течение двух минут, бой закончится, и победитель не будет выбран.");
            fighting.notify("У вас есть две минуты на бой, если вы не будете драться в течение двух минут, бой закончится, и победитель не будет выбран.");
            clearInterval(fightInterval);

            let fightTime = setTimeout(() => {

                if (player.getVariable("FIGHTING") === true && player.getVariable("FIGHTING_PLAYER") === fighting.id) {
                    user.stopFight(player, fighting, null);
                }
                clearTimeout(fightTime);
            }, 120000)
        }

    }, 1000);
};
user.stopFight = (player, target, winner) => {
    ...
    setTimeout(() => {
        ...
        let moneyBet = player.getVariable("FIGHTING_MONEY");

        player.setVariable("FIGHTING", false);
        player.setVariable("FIGHTING_PLAYER", null);
        player.setVariable("ROUNDS_WON", 0);

        target.setVariable("FIGHTING", false);
        target.setVariable("FIGHTING_PLAYER", null);
        target.setVariable("ROUNDS_WON", 0);

        target.setVariable("FIGHTING_MONEY", 0);
        player.setVariable("FIGHTING_MONEY", 0);

        if (winner !== null) {
            user.removeMoney(player, moneyBet, "Lost fist fight", 0);
            user.addMoney(winner, moneyBet, "Won Fist Fight", 0);

            player.notify(`${user.get(winner, 'name')} выиграл бой! Ты проиграла ${methods.moneyFormat(moneyBet)}`);
            winner.notify(`Вы выиграли битву с ${user.get(player, 'name')}. Вы получили ${methods.moneyFormat(moneyBet)}`);
        }
    }, 2500);
};
...
user.isInWeedZone = (player) => {
    let weedZoneFound = false;

    let zones = enums.WeedZones;

    for (let i = 0; i < zones.length; i++) {
        if (methods.distanceToPos(zones[i].position, player.position) < 15) {
            weedZoneFound = true; 
            break;
        }
    }
    return weedZoneFound;
};