                    ...
                    setTimeout(function () {
                        if (user.isLogin(player)) {
                            ...
                            if (vehicles.exists(veh)) {
                                ...
                                veh.position = new mp.Vector3(-69.44390106201172, 83.08438873291016, 71.19979095458984);
                                veh.heading = 63.10699462890625;
...
vShop.sendNotify = function(player, shopId, title, text) {
    if (user.isLogin(player)) {
        if (shopId === 14) {
            player.notifyWithPicture(`Real`, '~g~' + title, text, `CHAR_ASHLEY`, 2);
        } else {
            let shopItem = enums.carShopList[shopId];
...
vShop.rent = function(player, model, color1, color2, shopId) {
    ...
    if (shopId === 14) {
        ...
        }, new mp.Vector3(-69.44390106201172, 83.08438873291016, 71.19979095458984), 63.10699462890625, model);
    } else {
        vehicles.spawnCarCb(veh => {
