...
mp.events.addRemoteCounted('server:cont:vehicle', (player, veh) => {
    if (!user.isLogin(player))
        return;
    try {
        if (user.get(player, 'online_time') < 100) {
            let time = 100 - user.get(player, 'online_time');
            player.notify(`~r~Вам осталось отыграть ${time}`);
            return;
        }
        ...
        if (veh === 'bmw')
            user.giveVehicle(player, 'bmwx7', 1, false, '', false, true, slot);
        else if (veh === 'audi')
            user.giveVehicle(player, 'lx2018', 1, false, '', false, true, slot);
        else
            user.giveVehicle(player, 'esc21', 1, false, '', false, true, slot);
...
mp.events.addRemoteCounted('server:weedplants:check', (player, plantid) => {

    let pID = weedplants.getByID(plantid);
    if (pID) {
        player.call('client:weedplants:showmenu', [JSON.stringify(pID)]);
    }
});


mp.events.add('server:business:withdraw', async (player, businessid, bankaccount, card, amount) => {

    let businessData = business.getData(businessid);
    let creditCard = card;

    let businessBank = businessData.get('bank_id');
    let businessBankMoney = businessData.get('bank');
    let businessName = businessData.get('name');
    let nalog = coffer.getTaxBusiness();
    let businessCreditCard = businessData.get('bank_score');

    let bankList = ['Fleeca Bank', 'Blaine Bank', 'Pacific Bank', 'Maze Bank'];

    if (methods.bankFormat(businessCreditCard) != methods.bankFormat(creditCard)) return player.notify('Недействительная кредитная карта');
    if (bankaccount !== businessBank) return player.notify('Вы должны выбрать банк, в котором зарегистрирован ваш бизнес, чтобы продолжить. Ваш бизнес зарегистрирован по адресу ' + bankList[businessBank - 1]);

    if (user.get(player, 'bank_card') < 1) {
        player.notify(`~r~У вас нет банковской карты`);
        return;
    }
    if (businessBank === 0) {
        player.notify(`~r~Ваш бизнес не привязан ни к какому банку`);
        return;
    }
    let money = methods.parseFloat(amount);
    if (money > businessBankMoney) {
        player.notify(`~r~Недопустимая сумма`);
        return;
    }
    if (money < 1) {
        player.notify(`~r~Нельзя брать меньше 1`);
        return;
    }
    let bankTarif = 0;
    if (businessBank > 0)
        bankTarif = business.getPrice(businessid);

    business.addMoney(businessBank, money * (bankTarif / 100), 'Profit S. ' + businessName);
    business.removeMoney(businessid, money, 'Снятие ' + methods.bankFormat(user.get(player, 'bank_card')));
    business.save(businessid);

    user.addBankMoney(player, money * (100 - nalog) / 100, 'Вывод из бизнес-аккаунта ' + businessName);
    coffer.addMoney(1, money * nalog / 100);

    player.notify(`~b~Вы отказались ~s~${methods.moneyFormat(money * (100 - nalog) / 100)} ~b~со своего счета, включая налог`);
    player.notify(`~b~${bankTarif}% от суммы, отправленной в обслуживающий вас банк`);

});

mp.events.add('server:business:deposit', async (player, businessid, bankaccount, card, amount) => {
    //console.log(businessid, bankaccount, card, amount);
    let businessData = business.getData(businessid);
    let bankList = ['Fleeca Bank', 'Blaine Bank', 'Pacific Bank', 'Maze Bank'];
    let creditCard = card;
    let moneyAmount = amount;
    let userMoney = user.getBankMoney(player);
    let businessBank = businessData.get('bank_id');
    let businessCreditCard = businessData.get('bank_score');


    methods.debug('Business Credit Card: ' + businessCreditCard + ' | Business ID: ');
    ...
    if (moneyAmount > userMoney) {
        player.notify(`~r~У вас недостаточно денег на вашем банковском счете`);
        return;
    }
    if (moneyAmount < 1) {
        player.notify(`~r~Вы не можете внести менее 1 доллара`);
        return;
    }
    let money = methods.parseFloat(moneyAmount);

    business.addMoney(businessid, money, 'Зачислено на бизнес-счет из ' + methods.bankFormat(user.get(player, 'bank_card')));
    user.removeBankMoney(player, money, 'Депозит ' + businessData.get('name'));
    business.save(businessid);
    player.notify(`~b~Вы кладете деньги на бизнес-счет`);
});

mp.events.addRemoteCounted('server:weedplant:collect', (player, id) => {
    methods.debug('server:weedplant:collect');
    let pID = weedplants.getByID(id);
    if (pID) {
        player.call('server:weedplants:playscenario');
        player.notify('Сейчас вы собираете урожай, подождите...');
        setTimeout(() => {
            inventory.addItem(3, 1, inventory.types.Player, user.getId(player), pID.growngrams, 0, "{}");
            weedplants.destroy(id);
        }, 2500);
    }
});
...
mp.events.addRemoteCounted("server:vehicle:lockStatus", (player) => {
    ...
    try {
        if (player.vehicle && player.seat === 0 && player.vehicle.locked === false) {
...
mp.events.addRemoteCounted("onKeyPress:E", (player) => {
    ...
    houses.getAllHouses().forEach((val, key, object) => {
        if (methods.distanceToPos(player.position, val.position) < 1.5) {
            ...
            else player.call('client:showHouseOutMenu', [Array.from(houseData)]);
        }
...
mp.events.add("playerDeath", (player, reason, killer) => {
    ...
    if (user.isLogin(player) && player.getVariable("FIGHTING") === true) {
        if (typeof killer !== 'undefined' && killer.getVariable("FIGHTING") === true && killer.getVariable("FIGHTING_PLAYER") === player.id && player.getVariable("FIGHTING_PLAYER") === killer.id) {
            //fighting.setVariable("ROUNDS_WON", 0);
            let rounds = killer.getVariable("ROUNDS_WON");
            killer.setVariable("ROUNDS_WON", rounds + 1);
            if (rounds >= 2) {
                user.stopFight(player, killer, killer);
            }
            user.revive(player);
            user.revive(killer);
            user.showLoadDisplay(player);
            user.showLoadDisplay(killer);
            setTimeout(() => {
                user.hideLoadDisplay(player);
                user.hideLoadDisplay(killer);
            }, 2500);
        }
    }
